...
**LOGIC1 - V3 Scalping Mode (5-Minute Entries)**
- Entry timeframe: 5m
- Trend alignment required: 1H + 15M must agree
- Lot multiplier: 1.25x (aggressive)
- SL multiplier: 1.0x (standard)
- V3 signal routing: 5m signals → LOGIC1

**LOGIC2 - V3 Intraday Mode (15-Minute Entries)**
- Entry timeframe: 15m
- Trend alignment required: 1H + 15M must agree
- Lot multiplier: 1.0x (balanced)
- SL multiplier: 1.5x (wider)
- V3 signal routing: 15m signals → LOGIC2

**LOGIC3 - V3 Swing Mode (1-Hour Entries)**
- Entry timeframe: 1h
- Trend alignment required: 1D + 1H must agree
- Lot multiplier: 0.625x (conservative)
- SL multiplier: 2.5x (widest)
- V3 signal routing: 1h/4h signals, Screener Full, Golden Pocket → LOGIC3
...
**Hybrid Dual-Order Strategy with V3 Smart SL:**

The bot implements a V3-powered hybrid system where each entry signal creates two complementary orders:

**V3 Core Features:**
- **Multi-Timeframe Pillars:** Analysis across 15m, 1h, 4h, 1d timeframes
- **Trend Bypass:** Optional trend check bypass for high-conviction V3 entries
- **Consensus Scoring:** Minimum consensus score requirement (default: 5)
- **Signal Classification:**
  - Aggressive Reversal Signals: Liquidity_Trap_Reversal, Golden_Pocket_Flip, Screener_Full_Bullish/Bearish
  - Conservative Exit Signals: Bullish_Exit, Bearish_Exit

**Dynamic Signal Routing:**
| Signal Timeframe | Routed To | Purpose |
|------------------|-----------|----------|
| 5m | LOGIC1 (Scalping) | Fast entries |
| 15m | LOGIC2 (Intraday) | Balanced trades |
| 1h / 4h | LOGIC3 (Swing) | Larger moves |
| Screener Full | LOGIC3 Override | High-conviction all TFs |
| Golden Pocket (Higher TF) | LOGIC3 Override | Premium zones |

**Order Execution Strategy:**
- **Order A:** V3 Smart SL with pyramid re-entry capability
- **Order B:** Fixed $10 max risk SL with profit booking chains

Both orders work independently - if one fails, the other continues trading.
...
- **Order A (TP Trail)**: Uses V3 Smart SL system with TP continuation capability
- **Order B (Profit Trail)**: Uses adaptive SL calculated for $10 maximum risk (pip distance varies by lot size and symbol) with profit booking chain integration
...
| Level | Orders | Profit Target | Cumulative Orders | Status |
|-------|--------|---------------|-------------------|--------|
| 0 | 1 | $7 min | 1 | ✅ Enabled |
| 1 | 2 | $7 min each | 3 | ✅ Enabled |
| 2 | 4 | $7 min each | 7 | ✅ Enabled |
| 3 | 8 | $7 min each | 15 | ❌ Disabled (Config) |
| 4 | 16 | $7 min each | 31 | ❌ Disabled (Config) |

**Note:** Currently levels 0-2 are enabled. Levels 3-4 can be enabled via config (`profit_booking_config.enabled_levels`).
