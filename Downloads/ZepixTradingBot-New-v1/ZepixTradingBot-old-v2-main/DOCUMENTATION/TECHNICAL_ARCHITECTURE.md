...
### 4.1 V3 Integration System (`src/managers/dual_order_manager.py`)

**Purpose**: Multi-timeframe signal analysis and intelligent routing system

**Key Responsibilities**:
- Multi-Timeframe Pillar Analysis (15m, 1h, 4h, 1d)
- Consensus scoring (minimum score: 5)
- Signal classification (Aggressive Reversal vs Conservative Exit)
- Dynamic signal routing to LOGIC1/2/3
- Trend check bypass for high-conviction signals
- Hybrid order strategy coordination

**V3 Signal Routing**:
```python
signal_routing = {
    "tf_5m": "LOGIC1",      # Scalping mode
    "tf_15m": "LOGIC2",     # Intraday mode
    "tf_60m": "LOGIC3",     # Swing mode
    "tf_240m": "LOGIC3",    # Swing mode (4h)
    "screener_full_override": "LOGIC3",  # High conviction
    "golden_pocket_higher_tf_override": "LOGIC3"  # Premium zones
}
```
...
### 4.2 Trading Engine (`src/core/trading_engine.py`)
